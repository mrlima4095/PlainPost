async function autenticar(api) {
    ...
    try {
        const resposta = await fetch("https://servidordomal.fun/api/" + api, {
            method: "POST",
        ...
        if (resposta.status == 200 || resposta.status == 201) {
            ...
            Swal.fire("Erro", "Usuário ou senha incorretos!");
        } else if (resposta.status == 409) {
            Swal.fire("Erro", "Este nome de usuário já está em uso!");
        }
    } catch (erro) {
